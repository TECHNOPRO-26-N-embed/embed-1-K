...
|:--|:--|
| 作品タイトル | 回転機械の安全監視装置 |
| 一言説明（何ができるガジェットか） | rpm(回転数)を表示して設定した秒間回転数が超過したら警告を鳴らすガジェット |
...
| 接続するArduinoピン（予定） |  |
| 個人の設計と共通部品が協調する箇所 | 設定の秒間rpmを超えたら音を鳴らす |
...
```
【入力】                           　　　　　　　 【Arduino UNO R3】                【出力】
ロータリーエンコーダ（回転信号）─────→┐                                 　　　　┌──→ 4桁7セグメントディスプレイ（rpm表示）
...
[電源ON/初期化]
   ↓（初期化完了）
[待機中] ──（回転開始）──→ [計測中] ──（rpm表示更新）──（rpm設定値超過）──→ [警告中]
   ↑　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　 　   |
   └───────────────（リセットボタン）──────────────────────────────────────────┘
...
| 待機中 | モーターの回転開始を待ち、rpmを監視する | ディプレイ：0表示 |
| 計測中 | モーター回転中のrpmを計測し、設定値超過を監視 | ディスプレイ：rpm表示更新 |
| 警告中 | rpmが設定値を超過したため警告を出す | ディスプレイ：rpm表示継続 、ブザー：警告音を鳴らす |
| リセットボタン | リセットボタンは全状態で有効とし、押下時は最優先で待機中へ遷移する（キー入力）| ディスプレイ：0表示、ブザー：鳴っていれば停止 |
...
| currentState | 現在状態（0:待機 1:計測 2:警告） | int | 1B | 0 |  |
| pulseCount | 回転パルスのカウント値 | volatile uint16_t | 2B | 0 | 割り込みで加算 |
| lastCalcMillis | 回転数を再計算した時刻（ms） | unsigned long | 4B | 0 | millis運用 |
| rpmCurrent | 現在の回転数（rpm） | uint16_t | 2B | 0 |  |
| rpmThreshold | 警告しきい値（rpm） | uint16_t | 2B | 1500 | 変更可能 |
| displayValue | ディスプレイ表示値 | uint16_t | 2B | 0 | rpmCurrentを代入 |
| serialRxChar | PCから受信した文字 | char | 1B | 0 | 例: rでリセット |
...
|  | 回転数計算 | `calcRpm()` | m()	一定周期でrpmを計算し、状態を判定 | なし | なし | loop()内 |
|  | 表示更新 | `updateDisplay()` | rpm値をディスプレイに表示する | なし | なし | loop()内 |
|  | 警告制御 | `updateWarning()` | Warning()	rpmがしきい値超過時にブザーを鳴らす | なし | なし | loop()内 |
